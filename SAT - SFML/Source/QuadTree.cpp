...
void Quadtree::split(Objects** object, int maxShapes)
{
	...
	for (int i = 0; i < maxShapes; i++)
	{
		...
		if ((*object)->getPosition().x > topRightX - (*object)->getSize() && (*object)->getPosition().x < topRightX + width + (*object)->getSize() &&
			(*object)->getPosition().y > topRightY - (*object)->getSize() && (*object)->getPosition().y < topRightY + height + (*object)->getSize())
		{
			TRNum++;
			TRList.push_back(i);
		}

		if ((*object)->getPosition().x > bottomLeftX - (*object)->getSize() && (*object)->getPosition().x < bottomLeftX + width + (*object)->getSize() &&
			(*object)->getPosition().y > bottomLeftY - (*object)->getSize() && (*object)->getPosition().y < bottomLeftY + height + (*object)->getSize())
		{
			BLNum++;
			BLList.push_back(i);
		}

		if ((*object)->getPosition().x > bottomRightX - (*object)->getSize() && (*object)->getPosition().x < bottomRightX + width + (*object)->getSize() &&
			(*object)->getPosition().y > bottomRightY - (*object)->getSize() && (*object)->getPosition().y < bottomRightY + height + (*object)->getSize())
		{
			BRNum++;
			BRList.push_back(i);
		}
